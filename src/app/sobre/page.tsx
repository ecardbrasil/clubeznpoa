...
async function getLandingData(): Promise<{ featuredOffers: OfferCardData[]; partnerProfiles: Company[] }> {
  if (!isSupabaseMode) {
    initStorage();
    const data = getData();
    const offerCountByCompanyId = data.offers
    ...
    const featuredOffers: OfferCardData[] = data.offers
      .slice(0, 6)
      .map((o) => {
        const c = data.companies.find((c) => c.id === o.companyId);
        return {
          id: o.id,
          ...
          isFeatured: false,
          companyName: c?.publicName ?? c?.name ?? "Parceiro ClubeZN",
          images: o.images ?? [],
          partnerLogoImage: c?.logoImage,
          partnerCoverImage: c?.coverImage,
          partnerAddressLine: c?.addressLine,
    ...
    const partnerProfiles: Company[] = data.companies
      .sort((a, b) => {
...
export default async function SobrePage() {
  const { featuredOffers, partnerProfiles } = await getLandingData();
  return <SobrePageClient featuredOffers={featuredOffers} partnerProfiles={partnerProfiles} />;
}
