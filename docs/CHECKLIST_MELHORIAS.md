...
- [x] Aprovacao de empresas e ofertas no admin
- [x] Geração e validacao de codigo de resgate (6 digitos, 10 min)
- [x] Upload de fotos da oferta (ate 5) com selecao de capa
- [x] Perfil publico do parceiro com nome, endereco, bio e redes sociais
...
- [ ] Corrigir passivos de lint no `/partner` (regras de hooks/react compiler)
...
- [ ] Corrigir passivos de lint no `/partner`
  - Estimativa: 1,5 a 2 dias
  - Entrega: `npm run lint` sem erros no arquivo do parceiro e comportamento de formularios preservado
...
- [x] Melhorar feedback de erro/sucesso com toasts padronizados
  - Estimativa: 1 dia
  - Entrega: acoes principais com feedback consistente (perfil, oferta, validacao de codigo)
- [ ] Buffer de QA e ajustes
  - Estimativa: 1 dia
  - Entrega: validacao manual dos fluxos `consumer`, `partner` e `admin`

Total estimado Sprint 1: 4,5 a 5 dias uteis
...
Total estimado Sprint 2: 5 a 5,5 dias uteis

## Proximas melhorias (prioridade media)

- [ ] Persistencia real com backend (ex.: Supabase/Postgres)
- [ ] Autenticacao robusta (OTP celular + recuperacao de senha)
- [ ] Controle de permissao por papel no backend (RBAC)
- [ ] Auditoria de acoes sensiveis (aprovacoes, validacoes, alteracoes de perfil)
- [ ] Painel admin com filtros por bairro/categoria e periodo
- [x] Notificacoes de status para parceiro (empresa aprovada, oferta aprovada/rejeitada)
...
- [ ] Confianca e prova social
  ...
  - Adicionar depoimentos curtos e elementos de confianca
...
- [ ] Testes criticos cobrindo login, aprovacao, geracao e validacao de codigo
